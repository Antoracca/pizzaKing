    ...
    <section className="relative overflow-hidden bg-gradient-to-b from-neutral-50 via-white to-white py-16 sm:py-20 lg:py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-3 text-sm text-slate-600 sm:text-base lg:mt-4 lg:text-lg">
            De la sélection à la dégustation, tout est pensé pour un parcours
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span
                    className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${step.accent}`}
                  >
                    Étape {step.id.toString().padStart(2, '0')}
                  </span>
                  <span
                    className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${step.gradient} text-white shadow-lg`}
                  >
        ...
        <div className="mt-12 hidden gap-6 lg:mt-16 lg:grid lg:grid-cols-4 lg:gap-8">
          {STEPS.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${step.accent} lg:px-4 lg:py-1.5 lg:text-xs`}
                  >
                    Étape {step.id.toString().padStart(2, '0')}
                  </span>
                  <span
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${step.gradient} text-white shadow-lg lg:h-14 lg:w-14`}
                  >
                ...
                <div className="mt-7 space-y-3 lg:mt-8 lg:space-y-4">
                  ...
                  <p className="text-sm text-slate-600 lg:text-base lg:leading-relaxed">
                    {step.description}
                  </p>
                  <ul className="space-y-2.5 lg:space-y-3">
                    {step.highlights.map(highlight => (
                      <li
                        key={highlight}
                        className="flex items-start gap-2 text-sm text-slate-600 lg:text-base"
                      >
        ...
        <motion.div
          ...
          className="mt-12 flex flex-col items-center gap-4 rounded-3xl border border-orange-100 bg-white px-6 py-8 text-center shadow-lg lg:mt-16 lg:flex-row lg:justify-between lg:px-8 lg:py-10 lg:text-left"
        >
          ...
          <p className="text-sm text-slate-600 sm:text-base lg:max-w-2xl lg:text-lg">
            Le parcours a été pensé avec nos clients : interface adaptée mobile,
            notifications claires et service client humain qui réagit sous 2
            minutes.
          </p>
        </motion.div>
      </div>
